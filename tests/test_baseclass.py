...
@dataclass
class A(BaseClass, metaclass=BaseMetaclass):
    a: Optional[int] = None
    b: Optional[str] = None


@dataclass
class B(BaseClass, metaclass=BaseMetaclass):
    a: Optional[int] = None
    b: Optional[str] = None
...
def test_baseclass():
    ...
    with st:
        ...
        b2 = B(a=4, parent_id=b.id)
        assert b2.parent == b

        b3 = B(a=5, parent_id="929292")
...
def test_baseclass_relations():
    ...
    with st:
        ...
        b2 = B(a=2, parent_id="3")

        with pytest.raises(DoesNotExist):
            b2.parent

        b3 = B(a=2, parent_id="0")
        assert b3.parent == b1
